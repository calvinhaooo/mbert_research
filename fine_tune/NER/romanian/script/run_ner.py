...
class DataTrainingArguments:
    ...
    fold: int = field(
        metadata={"help": "Current fold of cross validation"}
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        train_data = train_splits[data_args.fold]
        # create dataset dict with the selected train and test data
    ...
    if training_args.do_predict:
        ...
        trainer.save_metrics(f"predict_fold_{data_args.fold}", metrics)
        # Save predictions
        output_predictions_file = os.path.join(training_args.output_dir, f"predictions_fold_{data_args.fold}.txt")
